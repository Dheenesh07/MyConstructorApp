  ...
  const toggleApproval = async (vendor) => {
    try {
      const updatedVendors = vendors.map(v => 
        v.id === vendor.id ? {...v, is_approved: !v.is_approved} : v
      );
      setVendors(updatedVendors);
      Alert.alert('Success', `Vendor ${vendor.is_approved ? 'suspended' : 'approved'} successfully!`);
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>🏗️ Admin Vendor Management</Text>
      ...
      <ScrollView style={styles.content}>
        {vendors.map(vendor => (
          <View key={vendor.id} style={styles.vendorCard}>
            ...
            <Text style={styles.vendorTax}>Tax ID: {vendor.tax_id}</Text>
            <View style={styles.ratingContainer}>
              <Ionicons name="star" size={16} color="#FFD700" />
              <Text style={styles.ratingText}>Rating: {vendor.rating}/5.0</Text>
            </View>
            <View style={styles.vendorActions}>
              ...
              <TouchableOpacity 
                style={[styles.actionBtn, { backgroundColor: vendor.is_approved ? '#FF9800' : '#4CAF50' }]} 
                onPress={() => toggleApproval(vendor)}
              >
...
const styles = StyleSheet.create({
  ...
  vendorPhone: { fontSize: 12, color: "#666", marginBottom: 2 },
  vendorTax: { fontSize: 12, color: "#666", marginBottom: 8 },
  ...
  actionBtn: { backgroundColor: "#003366", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6 },
